...
var shuffle = function () {
    for (var i = 0; colorCandidate.length > 0; i += 1) {
        color = color.concat(colorCandidate
            .splice(Math.floor(Math.random() * colorCandidate.length), 1));
    }
};
        ...
        card.addEventListener('click', function () {
            if (clickFlag && !completedCard.includes(this)) {
                ...
                if (clickCard.length === 2) {
                    var firstBackground = (clickCard[0].querySelector('.card-back')
                        .style
                        .backgroundColor);
                    var secondBackground = (clickCard[1].querySelector('.card-back')
                        .style
                        .backgroundColor);
